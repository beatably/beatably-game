...
/**
 * Load Swedish chart data from the local scraped JSON file.
 * Returns [] with a warning if the file is missing (run scrape-swedish-charts.js first).
 */
function loadSwedishChartData() {
  try {
    const p = config.swedish.localDataPath;
    dbg('Loading Swedish chart data from', p);
    if (fs.existsSync(p)) {
      const raw = fs.readFileSync(p, 'utf-8');
      const data = JSON.parse(raw);
      dbg('Loaded Swedish chart entries:', Array.isArray(data) ? data.length : 0);
      if (Array.isArray(data) && data.length && data[0].artist && data[0].title) {
        return data.map((row) => ({
          title: row.title,
          artist: row.artist,
    ...
    console.warn('[ChartProvider] Swedish chart data not found at', p, '— run backend/scripts/scrape-swedish-charts.js first');
  } catch (e) {
    console.warn('[ChartProvider] Failed to load Swedish chart data:', e.message);
  }
  return [];
}

/**
 * Get Swedish chart entries from local scraped data.
 * Supports year range filtering and difficulty ceilings (same as getChartEntries).
 */
async function getSwedishChartEntries({ difficulty = 'normal', yearMin, yearMax } = {}) {
  dbg('getSwedishChartEntries:', { difficulty, yearMin, yearMax });

  let entries = loadSwedishChartData();
  ...
  const deduped = dedupeByBestRank(entries);
  const filtered = filterByDifficulty(deduped, difficulty);
  dbg('Swedish: raw', entries.length, 'deduped', deduped.length, 'after difficulty', filtered.length);
